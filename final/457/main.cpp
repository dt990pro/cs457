...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "CS457-proj-final", NULL, NULL);
	if (window == NULL) {
	...
	Shader skyboxShader("skyShader.vs", "skyShader.fs");
	...
	unsigned int noise2 = loadTexture(getFullPath("c1.jpg").c_str());
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader.setFloat("Timer", time);
		shader.setVec3("cameraPos", camera.Position);
		shader.setVec3("light", lightPos);
		shader.setBool("use_b", use_b);
		shader.setBool("use_copper", use_copper);
...
void processInput(GLFWwindow *window) {
	...
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && space_down == false)
	{
		use_b = !use_b;
		space_down = true;
		if (use_b) printf("Blinn-Phong activated\n");
	}

	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE)
		space_down = false;

	if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS && c_down == false)
	{
		use_copper = !use_copper;
		c_down = true;
		if (use_copper) printf("copper activated\n");
	}
...
unsigned int loadCubemapsTexture(const std::vector<std::string>& fullpathVec) {
	...
	for (GLuint i = 0; i < fullpathVec.size(); ++i) {
		...
		if (data) {
			...
			std::cout << "fail to load: " << fullpathVec[i] << std::endl;
			stbi_image_free(data);
		}
	}
